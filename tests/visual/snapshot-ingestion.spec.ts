...
test('Snapshot Ingestion case study exposes the core architecture and current-state boundaries', async ({ page }) => {
  ...
  await expect(
    content.getByRole('heading', { name: 'What exists today and what is still missing', exact: true }),
  ).toBeVisible();
  await expect(
    content.getByText(/PostgreSQL is sufficient while the workload remains a bounded snapshot pipeline/),
  ).toBeVisible();
  await expect(content.getByText(/interview-oriented/i)).toHaveCount(0);
